...
func main() {
	...
	if err := database.DB.AutoMigrate(&models.Booking{}); err != nil {
	...
	go func() {
		utils.Logger.Info("Booking Service started", zap.String("port", cfg.Port))
	...
	defer cancel()
	srv.Shutdown(ctx)
	utils.Logger.Info("Booking Service stopped")
}
