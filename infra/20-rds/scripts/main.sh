...
# Example:
#   bash main.sh rds dev plan aws-secret-name [project bucket region]
#############################################################################################
R="\e[31m"
G="\e[32m"
Y="\e[33m"
...
if [[ $# -lt 4 ]]; then 
  print_error "Usage: bash main.sh <component> <env> <action> <aws-secret-name> [project bucket region]"
  print_info "Component: $VALID_COMPONENT (lowercase only)"
  print_info "Env: dev | qa | prod"
  print_info "Action: plan | apply | destroy"
  echo ""
  print_info "Example: bash main.sh $VALID_COMPONENT dev plan aws-secret-name [project bucket region]"
  exit 1
fi
...
if ! validate_item "$AWS_SECRET_NAME"; then 
  print_error "❌ AWS_SECRET_NAME not provided"
  print_info "Usage: bash main.sh <component> <env> <action> <aws-secret-name> [project bucket region]"
  exit 1
fi
...
# Fallback values
PROJECT="${5:-$(tf_output project)}"
BUCKET="${6:-$(tf_output bucket_id)}"
REGION="${7:-$(tf_output region)}"
