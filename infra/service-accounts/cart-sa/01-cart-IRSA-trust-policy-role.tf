data "aws_iam_policy_document" "cart_trust_policy" {
  statement {
...
resource "aws_iam_role" "cart_role" {
  name               = "${local.resource_name}-cart-role"
  assume_role_policy = data.aws_iam_policy_document.cart_trust_policy.json
